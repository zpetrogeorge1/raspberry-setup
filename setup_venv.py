...
def build_python():
    ...
    if not run_command("sudo apt update && sudo apt install -y build-essential zlib1g-dev libncurses5-dev "
                "libgdbm-dev libnss3-dev libss1-dev libreadline-dev libffi-dev curL",
                "Installing Python build dependencies"):
        return False
    # Build Python with limited jobs (-j2) for Raspberry Pi
    build_cmd = (f"mkdir -p {PYTHON_BUILD_DIR} && cd {PYTHON_BUILD_DIR} && "
...
def activate_and_setup():
    ...
    run_command("sudo apt install -y libatlas-base-dev gfortran", "Installing numpy/matplotlib dependencies")
